...
//   }
    ...
    {/* ? */}
    {/* {items===true? <div className="" onClick={()=> setItems(false)}>True</div>:<div className="" onClick={()=> setItems(true)}>false</div>} ,getBooksInfo(title,authr,ISBN)*/}
    {/* {check === true ? <button className="pa2 w4 bg-red" onClick={(e) => { 
      e.preventDefault();
      setCheck(false);
      getBooksInfo(title,authr,ISBN)}}>Sushi</button> */}
      {/* {check === true && title==="" && authr==="" && ISBN==="" ? <button className="pa2 w4 bg-light-gray" onClick={() => { setErr(true);setCheck(false);
      }}>Submit</button> : */}
    ...
    {/* <div className="pa2 w5 bg-yellow">Successfully Inserted</div> */}
      </div>
